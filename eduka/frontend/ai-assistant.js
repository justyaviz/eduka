...
async function load(){
  const [overview, leads, messages, faqs] = await Promise.all([
  ...
  $('#messagesList').innerHTML = messages.messages.length ? messages.messages.map(m=>`<div class="item"><b>${m.direction==='in'?'Kiruvchi':'Chiquvchi'} xabar</b><p>${(m.text||'').slice(0,220) || '-'}</p><div class="meta"><span class="pill ${m.direction==='in'?'amber':'green'}">${m.direction}</span><span class="pill">${fmtDate(m.created_at)}</span></div></div>`).join('') : '<div class="item"><b>Xabar yo‘q</b><p>Bot xabarlari shu yerda chiqadi.</p></div>';
