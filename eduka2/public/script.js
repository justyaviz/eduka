const plans = {
  monthly: {
    icon: 'calendar', title: 'Oylik (Kalendar)',
  ...
  group: {
    icon: 'users', title: 'Guruh boshlanish sanasi',
  ...
  course: {
    icon: 'credit', title: 'Kurs uchun to‘lov',
...
function renderPlan(key) {
  const p = plans[key];
  planCard.innerHTML = `
    <div class="plan-title"><div class="plan-emoji"><svg aria-hidden="true"><use href="#i-${p.icon}"></use></svg></div><h3>${p.title}</h3></div>
...
style.textContent = `.plan-emoji{width:92px;height:92px;display:grid;place-items:center;border-radius:22px;background:#edf3ff;color:#1E3AFF}.plan-emoji svg{width:44px;height:44px;stroke-width:2}`;
