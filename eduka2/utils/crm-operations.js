...
function calculatePayroll(rows,employee,month){
 const d=employee.data;const groups=new Set(rows.filter(r=>r.entity==='groups'&&r.data.teacher===employee.id).map(r=>r.id));
...
function register(router,{pool,initialized,allowed}){
 ...
 route('post','/payroll-run',async req=>{if(!allowed(req,'salary',true))throw fail('Ruxsat yo‘q',403);const {employee,month}=req.body;if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(month))throw fail('Oyni tanlang');return transaction(req,async(db,c)=>{
 ...
 route('post','/payroll-pay',async req=>{if(!allowed(req,'salary',true))throw fail('Ruxsat yo‘q',403);return transaction(req,async(db,c)=>{const r=(await db.query("SELECT * FROM eduka_records WHERE center_id=$1 AND id=$2 AND entity='salary' AND deleted=0",[c,req.body.id])).rows[0];if(!r)throw fail('Maosh topilmadi',404);if(r.data.paymentId)return {ok:true,paymentId:r.data.paymentId};if(r.data.calculation!=='v1')throw fail('Avval avtomatik maosh hisoblang');
